...
loaded_class1x <- read.table('/Users/kratikanaskulwar/GoogleDrive/VerafinProject/jupyter/VerafinCodefiles/InterpretabilityMethods/Local/class1XforR.csv', header = TRUE, sep = ",")
loaded_RFmodel <- readRDS('/Users/kratikanaskulwar/Desktop/R_CODES/R_RF.rds')

df_xtrain <- data.frame(X_train)
df_ytrain <- data.frame(y_train)
loaded_class1x <- data.frame(loaded_class1x)
...
datalist = list()
R_normlist = list()
datatovivo <- loaded_class1x[1:150,]

for (i in 1:nrow(datatovivo))
{
    profiles <- ceteris_paribus(explain_rf, loaded_class1x[i,])
...
write.csv(Result, file = "VIVO_fraud_originalweights.csv", row.names = FALSE)
write.csv(Result_norm, file = "VIVO_ALL_NORM_fraud.csv", row.names = FALSE)
write.csv(top5_norm, file = "VIVO_NORM_TOP5_fraud.csv", row.names = FALSE)
